...
// authored from the real headers (RuitkContext.h / RuitkSignal.h / RuitkPresence.h / RuitkRouter.h).
...
export const CORE_HOOKS: HookEntry[] = [
  {
    ...
    cppSignature: 'TTuple<T, TRuitkSetter<T>> FRuitkContext::UseState<T>(T Initial = T())',
...
<Button OnClicked={ SetCount(Count + 1) }>
  ...
  },
  {
    ...
    cppSignature: 'TTuple<T, TFunction<void(TAction)>> FRuitkContext::UseReducer<T, TAction>(TFunction<T(const T&, const TAction&)> Reducer, T Initial = T())',
...
<Button OnClicked={ Dispatch(ECartAction::Clear) }>
  ...
  },
  {
    name: 'UseEffect',
    category: 'core',
    signature: 'UseEffect([=]() { ... }, Ruitk::Deps(A, B))',
    cppSignature: 'template <typename TFn> void FRuitkContext::UseEffect(TFn&& Effect, FRuitkDeps Deps) — Effect returns void or FRuitkEffectCleanup',
    returns: 'nothing',
    description: 'Runs a passive side effect after the commit, in two passes across the tree: all pending cleanups first, then all setups. The dependency list gates re-runs: Ruitk::Deps(A, B) re-runs on change, empty Ruitk::Deps() runs once on mount, and Ruitk::EveryCommit() runs after every commit. Return an FRuitkEffectCleanup to release what the effect acquired — it runs before the next setup and on unmount.',
    example: `UseEffect([=]() -> FRuitkEffectCleanup
{
	auto Handle = Store->Subscribe(OnChange);
	return [=]() { Store->Unsubscribe(Handle); };
}, Ruitk::Deps(PlayerId));`,
    gotchas: [
      'Empty Ruitk::Deps() is mount-only; Ruitk::EveryCommit() is after every commit — they are different sentinels.',
      'The lambda may return void or an FRuitkEffectCleanup; the dispatch is if-constexpr, so either form compiles unchanged.',
  ...
  },
  {
    ...
    category: 'core',
    signature: 'UseLayoutEffect([=]() { ... }, Ruitk::Deps(A, B))',
    cppSignature: 'template <typename TFn> void FRuitkContext::UseLayoutEffect(TFn&& Effect, FRuitkDeps Deps) — Effect returns void or FRuitkEffectCleanup',
    returns: 'nothing',
    ...
    example: `UseLayoutEffect([=]() { SetMeasuredWidth(MeasurePanel()); }, Ruitk::EveryCommit());`,
    gotchas: [
  ...
  },
  {
    name: 'UseMemo',
    category: 'core',
    signature: 'const T& Value = UseMemo<T>(Factory, Ruitk::Deps(A, B))',
    cppSignature: 'const T& FRuitkContext::UseMemo<T>(TFunction<T()> Factory, FRuitkDeps Deps)',
    returns: 'a reference to the cached value',
    description: 'Caches an expensive computed value across renders, re-running the factory only when the dependency list changes. The comparison is shallow, per Ruitk::Deps semantics. The value lives in the hook cell and is returned by const reference, so unchanged renders pay no copy.',
    example: `const TArray<FString>& Sorted = UseMemo<TArray<FString>>(
	[&]() { return SortItems(Items, Filter); }, Ruitk::Deps(Filter));
...
<ListPanel Items={ Sorted } />`,
  ...
  },
  {
    name: 'UseCallback',
    category: 'core',
    signature: 'FRuitkCallback Cb = UseCallback(Fn, Ruitk::Deps(A, B))',
    cppSignature: 'FRuitkCallback FRuitkContext::UseCallback(TFunction<void()> Fn, FRuitkDeps Deps) — payload overload takes TFunction<void(const FRuitkValue&)>',
    returns: 'an FRuitkCallback that keeps its identity while deps are unchanged',
    description: 'Caches a callback identity across renders by deps — the memo-friendly way to pass event handlers to children without breaking their prop equality. Two overloads accept a zero-argument lambda or one taking the FRuitkValue event payload. Implemented directly over UseMemo.',
    example: `FRuitkCallback OnSave = UseCallback([=]() { SaveDocument(DocumentId); }, Ruitk::Deps(DocumentId));
...
<SaveButton OnSave={ OnSave } />`,
  ...
  },
  {
    ...
    cppSignature: 'TSharedRef<TRuitkRef<T>> FRuitkContext::UseRef<T>(T Initial = T())',
...
<Button OnClicked={ ClickCount->Current++ }>
  ...
  },
  {
    ...
    category: 'core',
    signature: 'const T& Handle = UseImperativeHandle<T>(Factory, Ruitk::Deps(A, B))',
    cppSignature: 'const T& FRuitkContext::UseImperativeHandle<T>(TFunction<T()> Factory, FRuitkDeps Deps)',
    returns: 'a reference to the cached handle object',
    description: 'Builds an imperative handle a parent can call into, rebuilt only when deps change. Two forms: the memo alias (shown above), and the ref-PUBLISHING form UseImperativeHandle(TargetRef, Factory, Deps) — it writes the handle into a parent-supplied TRuitkRef on mount and clears it on unmount (the React shape; both siblings ship it). Pair with Ruitk::Slate::WidgetFromHandle<TWidget>(Handle) to call live Slate APIs (ScrollToEnd()-class) through a captured host Ref.',
    example: `const FPanelHandle& Handle = UseImperativeHandle<FPanelHandle>(
	[=]() { return FPanelHandle{ FocusField, ScrollToTop }; }, Ruitk::Deps());

<InspectorHost Handle={ Handle } />

// Ref-publishing form: the parent passed us Props.PanelRef (TSharedRef<TRuitkRef<FPanelHandle>>)
UseImperativeHandle<FPanelHandle>(Props.PanelRef,
  ...
  },
  {
    ...
    cppSignature: 'const T& FRuitkContext::UseContext<T>(const TRuitkContext<T>& Handle)',
...
<TextBlock Text="Themed label" ColorAndOpacity={ Theme } />`,
  ...
  },
  {
    ...
    cppSignature: 'void FRuitkContext::ProvideContext<T>(const TRuitkContext<T>& Handle, T Value)',
    returns: 'nothing',
...
<VerticalBox>
  ...
  },
  {
    ...
    category: 'core',
    signature: 'FRuitkCallback Cb = UseStableCallback(Fn)',
    cppSignature: 'FRuitkCallback FRuitkContext::UseStableCallback(TFunction<void()> Fn)',
    returns: 'an FRuitkCallback whose identity never changes for the component lifetime',
    ...
    example: `FRuitkCallback OnPoll = UseStableCallback([=]() { SetStatus(QueryStatus(ServerId)); });

UseEffect([=]() -> FRuitkEffectCleanup
{
  ...
  },
  {
    name: 'UseStableFunc',
    category: 'core',
    signature: 'FRuitkCallback Cb = UseStableFunc(Fn)',
    cppSignature: 'FRuitkCallback FRuitkContext::UseStableFunc(TFunction<void()> Fn)',
    returns: 'an FRuitkCallback whose identity never changes for the component lifetime',
    ...
    example: `FRuitkCallback OnRefresh = UseStableFunc([=]() { SetTick(Tick + 1); });`,
    gotchas: [
  ...
  },
  {
    ...
    category: 'core',
    signature: 'FRuitkCallback Cb = UseStableAction(Fn)',
    cppSignature: 'FRuitkCallback FRuitkContext::UseStableAction(TFunction<void(const FRuitkValue&)> Fn)',
    returns: 'an FRuitkCallback whose identity never changes, invoked with the event payload',
    description: 'The payload-taking form of the stable-callback family: a fixed-identity FRuitkCallback whose body receives the FRuitkValue argument the caller passes. Like its siblings, the wrapper identity never changes while the inner closure is replaced every render. UseStableCallback and UseStableFunc are built on top of it.',
    example: `FRuitkCallback OnPicked = UseStableAction([=](const FRuitkValue& Value)
{
  ...
  },
  {
    ...
    cppSignature: 'T FRuitkContext::UseDeferredValue<T>(T Value, FRuitkDeps Deps = FRuitkDeps())',
...
<SearchResults Query={ Deferred } />`,
    gotchas: [
      'Without explicit deps, change detection is operator== on the value itself; pass Ruitk::Deps(...) to control it.',
  ...
  },
  {
    ...
    cppSignature: 'TTuple<bool, TFunction<void(TFunction<void()>)>> FRuitkContext::UseTransition()',
...
<Button OnClicked={ StartTransition([=]() { SetTab(ETab::Details); }) }>
  ...
  },
  {
    name: 'UseSignal',
    category: 'core',
    signature: 'T Value = Ruitk::UseSignal<T>(Ctx, Sig)   // selector form: Ruitk::UseSignal<T, TSelected>(Ctx, Sig, Selector)',
    cppSignature: 'TSelected Ruitk::UseSignal<T, TSelected>(FRuitkContext& Ctx, const TSharedRef<TRuitkSignal<T>>& Sig, TFunction<TSelected(const T&)> Selector) — selectorless overload returns the whole T',
    returns: 'the signal\'s current value (or the selected slice)',
    description: 'Subscribes the component to a TRuitkSignal — a reactive value store that lives outside the component tree. It follows React\'s useSyncExternalStore discipline: the render reads the snapshot directly from the store, and the subscription happens in a passive effect with a post-subscribe re-check, so a change landing between render and effect is never missed. With a selector, the component re-renders only when the selected slice changes.',
    example: `const int32 Score = Ruitk::UseSignal<int32>(Ctx, Game::GScoreSignal);

<TextBlock Text={ Ruitk::Fmt(TEXT("Score: {}"), Score) } />`,
    gotchas: [
      'Written with explicit Ctx in markup — Ruitk::UseSignal(Ctx, ...); only the built-in member hooks are called bare.',
  ...
  },
  {
    name: 'UseSignalKey',
    category: 'core',
    signature: 'T Value = Ruitk::UseSignalKey<T>(Ctx, Key, Initial)',
    cppSignature: 'T Ruitk::UseSignalKey<T>(FRuitkContext& Ctx, FName Key, T Initial = T())',
    ...
    example: `const int32 Count = Ruitk::UseSignalKey<int32>(Ctx, TEXT("Demo.Counter"), 0);

<TextBlock Text={ Ruitk::Fmt(TEXT("Count: {}"), Count) } />`,
    gotchas: [
      'Keyed signals OUTLIVE their readers; Ruitk::ClearSignals() is the full session reset (subscribers are not notified).',
      'Reading one key as two different types trips a runtime type check — the registry is honest about misuse, never silent.',
      'Written with explicit Ctx in markup — Ruitk::UseSignalKey(Ctx, ...).',
    ],
  },
  {
    name: 'UseTween',
    ...
    cppSignature: 'float FRuitkContext::UseTween(float Target, float DurationSec = 0.25f, ERuitkEase Ease = ERuitkEase::InOut)',
    ...
    example: `const float Opacity = UseTween(bVisible ? 1.0f : 0.25f, 0.3f, ERuitkEase::Out);
...
<Border RenderOpacity={ Opacity }>
  ...
  },
  {
    ...
    cppSignature: 'float FRuitkContext::UseAnimate(bool bIn, float DurationSec = 0.25f, ERuitkEase Ease = ERuitkEase::InOut)',
    ...
    example: `FRuitkPresenceState P = UsePresence(Ctx);
...
<Border RenderOpacity={ T }>
  ...
  },
  {
    ...
    cppSignature: 'T FRuitkContext::UseTweenValue<T>(const T& Target, float DurationSec = 0.25f, ERuitkEase Ease = ERuitkEase::InOut)',
...
<Border BorderImage="WhiteBrush" BorderBackgroundColor={ Tint }>
    ...
    gotchas: [
      'T needs an Ruitk::LerpTween overload — float, FVector2D, and FLinearColor ship in the box.',
  ...
  },
  {
    name: 'UseSfx',
    category: 'core',
    signature: 'FRuitkCallback Play = UseSfx(Bus)',
    cppSignature: 'FRuitkCallback FRuitkContext::UseSfx(FName Bus = NAME_None)',
    returns: 'a play-sound callback bound to the named bus',
    description: 'Returns a callback that fires a sound request on a named bus. Dispatch goes through the process-wide sfx sink registered with Ruitk::SetSfxSink — the game decides how a bus maps to audio assets and world context, keeping the UI code declarative. The FRuitkValue payload handed to the callback rides through to the sink untouched.',
    example: `FRuitkCallback PlayClick = UseSfx(TEXT("UI.Click"));
...
<Button OnClicked={ PlayClick.Execute() }>
  ...
  },
  {
    name: 'UseSafeArea',
    category: 'core',
    signature: 'FRuitkSafeArea Insets = UseSafeArea()',
    cppSignature: 'FRuitkSafeArea FRuitkContext::UseSafeArea()',
    returns: 'the device safe-area insets in pixels (Left, Top, Right, Bottom)',
    description: 'Reads the display safe-area insets so content can stay clear of notches and rounded corners. The values are host-supplied through an owner chain: a mock test value, then Slate\'s FDisplayMetrics, then a CommonUI override when that module is active. Returns an FRuitkSafeArea struct of four floats in pixels.',
    example: `FRuitkSafeArea Safe = UseSafeArea();
...
<Border Padding={ FMargin(Safe.Left, Safe.Top, Safe.Right, Safe.Bottom) }>
  ...
  },
  {
    name: 'UsePresence',
    category: 'core',
    signature: 'FRuitkPresenceState P = UsePresence(Ctx)',
    cppSignature: 'FRuitkPresenceState UsePresence(FRuitkContext& Ctx)   // global free function, not a Ctx member',
...
<Border RenderOpacity={ T }>
    ...
    gotchas: [
      'Written with explicit Ctx (UsePresence(Ctx)) — it is a free function, not a Ctx member and not in Ruitk::.',
...
export const ROUTER_HOOKS: HookEntry[] = [
  {
    ...
    cppSignature: 'TFunction<void(const FString& To, bool bReplace)> UseNavigate(FRuitkContext& Ctx)',
  ...
  },
  {
    ...
    cppSignature: 'TFunction<void(int32 Delta)> UseGo(FRuitkContext& Ctx)',
  ...
  },
  {
    ...
    cppSignature: 'TTuple<bool, TFunction<void()>> UseBackStack(FRuitkContext& Ctx)',
  ...
  },
  {
    name: 'UseLocation',
    category: 'router',
    signature: 'const FRuitkLocation& Loc = UseLocation()',
    cppSignature: 'const FRuitkLocation& UseLocation(FRuitkContext& Ctx)',
    returns: 'a const reference to the current FRuitkLocation (Pathname, Search, Hash, Key)',
    ...
    example: `const FRuitkLocation& Loc = UseLocation();\n\n<TextBlock Text={ Loc.ToHref() } />`,
    gotchas: [
  ...
  },
  {
    ...
    cppSignature: 'FString UsePathname(FRuitkContext& Ctx)',
  ...
  },
  {
    ...
    cppSignature: 'FString UseSearch(FRuitkContext& Ctx)',
    returns: 'the current search string including the leading "?" (or empty)',
    description: 'Convenience over UseLocation: the current raw search string, with the leading ? included (empty when there is no query). Pair it with the pure helper Ruitk::ParseSearch to get a key-value map, or reach straight for UseSearchParams for the parsed read-write surface.',
  ...
  },
  {
    ...
    cppSignature: 'const TMap<FString, FString>& UseParams(FRuitkContext& Ctx)',
  ...
  },
  {
    ...
    cppSignature: 'TTuple<TMap<FString, FString>, TFunction<void(TMap<FString, FString>)>> UseSearchParams(FRuitkContext& Ctx)',
  ...
  },
  {
    name: 'UseMatch',
    category: 'router',
    signature: 'FRuitkPathMatch M = UseMatch(TEXT("/users/:id"))',
    cppSignature: 'FRuitkPathMatch UseMatch(FRuitkContext& Ctx, const FString& Pattern)',
    returns: 'an FRuitkPathMatch: bMatched (also via explicit operator bool), Params, the matched Pathname',
  ...
  },
  {
    ...
    cppSignature: 'bool UseIsActive(FRuitkContext& Ctx, const FString& Pattern, bool bEnd = false)',
  ...
  },
  {
    ...
    cppSignature: 'FString UseResolvedPath(FRuitkContext& Ctx, const FString& To)',
  ...
  },
  {
    ...
    cppSignature: 'FString UseHref(FRuitkContext& Ctx, const FString& To)',
  ...
  },
  {
    name: 'UseOutlet',
    category: 'router',
    signature: 'FRuitkNode Child = UseOutlet()',
    cppSignature: 'FRuitkNode UseOutlet(FRuitkContext& Ctx)',
    returns: 'the nested matched child element as an FRuitkNode (an empty Fragment when there is none)',
    ...
    example: `export FRuitkNode SettingsLayout() {\n\treturn (\n\t\t<VerticalBox>\n\t\t\t<TextBlock Text="Settings" />\n\t\t\t{ UseOutlet() }\n\t\t</VerticalBox>\n\t);\n}`,
    gotchas: [
  ...
  },
  {
    name: 'UseRoutes',
    category: 'router',
    signature: 'FRuitkNode Matched = UseRoutes(RouteList)',
    cppSignature: 'FRuitkNode UseRoutes(FRuitkContext& Ctx, const TArray<Ruitk::FRuitkRoute>& RouteList)',
    ...
    example: `TArray<Ruitk::FRuitkRoute> RouteList;\nRouteList.Add(Ruitk::FRuitkRoute{ TEXT("/"),          HomeScreen() });\nRouteList.Add(Ruitk::FRuitkRoute{ TEXT("/users/:id"), UserScreen() });\n\n<Overlay>{ UseRoutes(RouteList) }</Overlay>`,
    gotchas: [
      'Must run under a Router — without one the location is the default "/" and only a route matching "/" can render.',
      'Ruitk::Routes(RouteList) is the node-shaped wrapper over this hook; use whichever fits the call site.',
    ],
  },
  {
    name: 'UseNavigationType',
    category: 'router',
    signature: 'ERuitkNavigationType NavType = UseNavigationType()',
    cppSignature: 'ERuitkNavigationType UseNavigationType(FRuitkContext& Ctx)',
    returns: 'ERuitkNavigationType::Push, ::Replace, or ::Pop',
    ...
    example: `ERuitkNavigationType NavType = UseNavigationType();\n\n<TextBlock Text={ NavType == ERuitkNavigationType::Pop ? TEXT("went back") : TEXT("went forward") } />`,
    gotchas: [
  ...
  },
  {
    ...
    cppSignature: 'bool UseInRouterContext(FRuitkContext& Ctx)',
  ...
  },
  {
    ...
    cppSignature: 'void UseBlocker(FRuitkContext& Ctx, bool bBlock, TFunction<void(const FString& AttemptedHref)> OnBlocked)',
