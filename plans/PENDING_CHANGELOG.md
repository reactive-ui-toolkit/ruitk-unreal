...
- [lane A] [plugin] **THE FLIP (family-parity M8): `ruitk.TimeSlicing` now defaults ON** — the family defaults land: render passes run as time-sliced scheduler actions (2.0 ms quantum per `ruitk.TimeSliceMs`, 4.0 ms per-frame budget per `ruitk.FrameBudgetMs`, commits atomic, work spread across frames); mount and every FlushSync surface (host widgets, item-model row roots, HMR, preview) stay fully synchronous by design, so first frames are identical; opt out with `bTimeSlicing=false` in Project Settings ▸ Reactive UI Toolkit (or `ruitk.TimeSlicing 0`) for the scheduler-bypass single-pass world — both worlds battery-green 167/167, bench shows no regression on any scenario (the doom_reconcile_frame headline measured faster than both before-rows) (PR #53)
