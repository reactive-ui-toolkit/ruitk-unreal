...
- [lane A] [plugin] TB-13/TB-15 HMR correctness: hook-shape edits now RESET state cleanly across Live Coding patches (family rule restored; per-cell type identity makes any shape mismatch memory-safe — was a crash); value exports lower as inline functions so style/value edits actually apply under HMR (global initializers never re-run on patch); HMR error rows coalesce (×N) and the counter reads current errors (rounds 16, TESTING_BUGS TB-13/14/15)
- [lane B] [both extensions] live UETKX2305 strict-usage mirror (a usage whose name a workspace file exports but this file never imports errors as-you-type, with the add-import quick-fix; kind-matched, sidecar-deduped) + 2310 near-miss candidates extended to import names and same-file decls — an orphaned usage next to a misspelled import now points at the import (TESTING_BUGS TB-19)
- [lane A] [plugin] FILE_SCOPED_EXPORTS: every `.uetkx` file is its own module — same-name exports across files are LEGAL (ES semantics, family parity with the Unity sibling). Every decl emits inside a per-file flat namespace; runtime/HMR identity is the file-qualified name with short-name suffix resolution at the designer edges (ComponentName fields, MountNamed, preview — ambiguity is loud, never first-wins); UETKX2106 retired (number reserved), UETKX2303 re-keyed to local bindings, NEW UETKX2329 case-folded-FQN check; CodegenVersion 4 (full regenerate + one-time remount on upgrade); new `RUI::ResolveNamed`/`GetRegisteredFactoryNames` APIs (TB-20, plans/archive/FILE_SCOPED_EXPORTS_PLAN.md)
- [lane B] [both extensions] FILE_SCOPED_EXPORTS mirrors: the live 2106 mirror retired (same-name export pairs publish zero diagnostics); 2305 add-import suggestions and findExporter pick the importer-NEAREST exporter exactly like the compiler; the rename refusal for names exported elsewhere is retired (per-touched-file collision checks remain)
- [lane A] [plugin] TB-21/TB-23 Live-Coding-safe codegen + registries: process-global registries are zero-initialized namespace-scope pointers with first-touch allocation (function-local statics DUPLICATE across Live Coding patches — post-patch registrations landed in a dead copy and old fibers dereferenced freed trees at PIE stop); every component emits a STABLE `<Name>_UetkxImpl` shim (the registered pointer + Live Coding redirect anchor) calling a content-hashed `<Name>_UetkxBody_<hash>` that owns all lambdas (lambdas mangle by ORDINAL — a markup edit shifted later ordinals and old closures ran wrong-layout destructors); hooks/utils get the same forwarder + hashed-body shape (rounds 16+, TESTING_BUGS TB-21/TB-23)
- [lane A] [plugin] TB-26 HMR patch toasts coalesce: rapid saves updated ONE notification in place (text + restarted expiry) instead of stacking fading toasts into a blurry smear; the toast carries the running patch total (TB-26)
- [lane A] [plugin] TB-28 `return null;` render-nothing (family parity): both bare and `return ( null );` forms are first-class component returns — scanners classify them, codegen lowers to an empty node array, the formatter canonicalizes to bare `return null;`, and the 2101/3007 messages name the form; per-leg corpus cases + formatter goldens + contract fixture pin it (TB-28, TD-035)
- [lane B] [both extensions] TB-22 watched-files revalidation: the client watches `**/*.uetkx`, so deleting/renaming a file ON DISK re-flags open importers immediately (previously nothing updated until a keystroke) (TB-22)
- [lane B] [both extensions] TB-24 live tag policing: an unimported component tag errors as-you-type (2305 with the add-import quick-fix, importer-nearest exporter) and an unknown tag gets 2307 — previously tags were only checked by the hash-gated compiler sidecar, so the squiggle flickered with every edit (TB-24)
- [lane B] [both extensions] TB-25 import-specifier DX: single-quoted specifiers color and get full tooling in both quote styles (formatter still canonicalizes to `"`); specifier completions order nearest-first; accepting a suggestion REPLACES the typed prefix instead of appending (TB-25)
- [lane B] [both extensions] TB-27 embedded-C++ scaffold returns FRuiNode: clangd no longer flags every early return (`return ( <X/> );`, `return (<></>);`) with "void function should not return a value" — the virtual doc's setup wrapper was void while early-return windows kept their return statements (TB-27)
- [lane B] [both extensions] fragment delimiters `<>`/`</>` color as tag punctuation like element brackets (they fell through to the C++ operator rule; new tag-fragment grammar region with full children recursion, both grammar copies byte-identical) (round 16b)
