# ruitk-unreal — Master Plan (AI-first)
...
semantics: ReactiveUIToolKit (Unity C#) is the behavioral reference, Reactive UI Toolkit for Godot is the
...
- **D-01 — Names & headers.** Plugin folder `Plugins/ReactiveUIToolkit/`, descriptor `ReactiveUIToolkit.uplugin`,
  FriendlyName "Reactive UI Toolkit for Unreal" (final store name is owner's call — §7 Q1, .NET-Reactive UI Toolkit
...
- **D-23 — IDE intelligence: third instantiation of the virtualDoc + length-preserving SourceMap
  ...
  (`#include` preamble spliced verbatim, `#include "Reactive UI Toolkit/HookStubs.h"` — a REAL shipped
...
- **D-29 — Distribution: GitHub-first (MIT), Fab free listing as the funnel** (Cesium/Sentry
  ...
  `Reactive UI Toolkit-<ver>-UE<engine>.zip` per engine version via `RunUAT BuildPlugin -StrictIncludes`
...
- **D-32 — Legal & licensing.** (a) **Generated code in USER projects belongs to the user**: the
  ...
  `// Generated by Reactive UI Toolkit from Foo.uetkx — this generated code belongs to your project.`
...
ruitk-unreal/                          # repo root IS the demo host project (Godot-repo pattern)
  ...
  RuitkUnrealDocs~/                    # DELIVERABLE 3 — docs site (React+Vite, cloned family shell)
...
- **Steps:**
  ...
  6. CI `test.yml`: job matrix armed by repo variables (the Godot "inert until armed" pattern):
     ...
     before Phase 5, `RuitkUnrealDocs~/` content before Phase 8) and exits 0 with a "not armed yet" log line,
  ...
  8. Docs scaffold: clone the family docs-site shell into `RuitkUnrealDocs~/` (routing/theme/search), landing
...
- **Files touched:** everything in §2 except module internals; `.github/workflows/*`;
  `.claude/skills/*`; `templates/*`; `RuitkUnrealDocs~/*` shell.
...
- **Status:** CODE-COMPLETE 2026-07-10 (owner PIE playtest pending — the field-test touchpoint): 15 widgets + style v1 (pointer-identity test green) + GO-05 pool (same-pointer reuse test) + focus fences + STATGROUP; 38/38 suites incl. Reactive UI Toolkit.{Slate,Widgets,Style,Demos}; reorder spike decided (TD-010, rows in BENCH_BASELINES.md). Header-sweep audit mapped remaining widget surface to TD-012.
...
- **Steps:**
  ...
  6. **Editor UX / onboarding surface** (the D-27 "asset actions" get built here — Phase 8's
     ...
     Reactive UI Toolkit → New Component (writes the template `.uetkx` + triggers the sweep), double-click
...
- **Status:** PARTIAL 2026-07-11. Done: the demo story (all 11 gallery screens compile from
  ...
  cycle (content/branding decisions): the RuitkUnrealDocs~ site (React/Vite family
...
1. **Naming — ANSWERED: "ruitk-unreal" stays** (repo name already is; plugin FriendlyName
