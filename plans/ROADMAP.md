...
| 9 | Release & publishing | IN PROGRESS | GitHub releases live through plugin 0.13.0 / extensions 0.7.0 (publish.yml, tag-gated); license settled (ReactiveUI Community License). Remaining, in order: the HMR field test ([HMR_FIELD_TEST.md](HMR_FIELD_TEST.md)) → drain `PENDING_CHANGELOG.md` (rounds 10–15 staged) via release-process → per-engine zips + packaged-fidelity test → Fab listing/upload + demo video + Discord (owner-gated) |
