...
//
// Ruitk.Bench.Core — reconciler-only microbenches over the MOCK host (no Slate, no
// widgets: pure render/diff/commit cost). NOT pass/fail (family: bench*.gd) — every run
// logs "RUITKBENCH ..." rows; committed numbers live in plans/BENCH_BASELINES.md with their
...
static FRuitkNodeArray BenchLeafList(FRuitkContext& Ctx, const FRuitkEmptyProps&, const TArray<FRuitkNode>&)
{
	auto [Frame, SetFrameState] = Ctx.UseState<int32>(0);
	RuitkBench::SetFrame = SetFrameState;
	const TArray<FString>& Labels = RuitkBench::Labels();
	TArray<FRuitkNode> Leaves;
	Leaves.Reserve(RuitkBench::LeafCount);
...
RUITK_COMPONENT(BenchLeafList)

/** 500 keyed boxes whose order a state flips — the keyed mark-sweep + reorder path. */
static FRuitkNodeArray BenchKeyedList(FRuitkContext& Ctx, const FRuitkEmptyProps&, const TArray<FRuitkNode>&)
{
	auto [bReversed, SetReversedState] = Ctx.UseState<bool>(false);
	RuitkBench::SetReversed = SetReversedState;
	const TArray<FString>& Labels = RuitkBench::Labels();
	TArray<FRuitkNode> Leaves;
	Leaves.Reserve(RuitkBench::KeyedCount);
	for (int32 n = 0; n < RuitkBench::KeyedCount; ++n)
	{
		const int32 i = bReversed ? (RuitkBench::KeyedCount - 1 - n) : n;
		Leaves.Add(RuitkTest::Box(RuitkTest::BoxProps(Labels[i], i), {}, FRuitkKey(i)));
	}
	return {RuitkTest::Box(RuitkTest::BoxProps(TEXT("list")), MoveTemp(Leaves))};
}
RUITK_COMPONENT(BenchKeyedList)

/** Small nested tree (containers + leaves + a hook) for mount/unmount churn. */
static FRuitkNodeArray BenchChurnTree(FRuitkContext& Ctx, const FRuitkEmptyProps&, const TArray<FRuitkNode>&)
{
	auto [V, SetV] = Ctx.UseState<int32>(0);
	(void)SetV;
	const TArray<FString>& Labels = RuitkBench::Labels();
	TArray<FRuitkNode> Rows;
	Rows.Reserve(RuitkBench::ChurnLeaves / 10);
	for (int32 r = 0; r < RuitkBench::ChurnLeaves / 10; ++r)
	{
		TArray<FRuitkNode> Cells;
		Cells.Reserve(10);
		for (int32 c = 0; c < 10; ++c)
		{
			Cells.Add(RuitkTest::Box(RuitkTest::BoxProps(Labels[r * 10 + c], V)));
		}
		Rows.Add(RuitkTest::Box(RuitkTest::BoxProps(Labels[r], r), MoveTemp(Cells)));
	}
	return {RuitkTest::Box(RuitkTest::BoxProps(TEXT("grid")), MoveTemp(Rows))};
}
RUITK_COMPONENT(BenchChurnTree)

// ── driver ────────────────────────────────────────────────────────────────────────────────

IMPLEMENT_SIMPLE_AUTOMATION_TEST(FRuitkBenchCoreTest, "Ruitk.Bench.Core",
								 EAutomationTestFlags::EditorContext | EAutomationTestFlags::PerfFilter)
bool FRuitkBenchCoreTest::RunTest(const FString&)
{
	...
	// mount_1000_leaves: full cold mount (render + fibers + host nodes), teardown untimed.
	{
		RuitkBench::Reset();
		TArray<double> Us;
		for (int32 r = 0; r < RuitkBench::Reps; ++r)
		{
			FRuitkTestHarness H;
			const uint64 T0 = FPlatformTime::Cycles64();
			H.Mount(Ruitk::FC(&BenchLeafList));
			Us.Add(RuitkBench::ToUs(FPlatformTime::Cycles64() - T0));
		}
	...
	// update_1_of_1000: one leaf's label changes per frame — render + diff + 1 CommitUpdate.
	{
		...
		for (int32 r = 0; r < RuitkBench::Reps; ++r)
		{
			RuitkBench::ChangedIndex = (r * 137) % RuitkBench::LeafCount;
			const uint64 T0 = FPlatformTime::Cycles64();
			RuitkBench::SetFrame(r + 1);
			H.Host.PumpFrame();
			Us.Add(RuitkBench::ToUs(FPlatformTime::Cycles64() - T0));
		}
	...
	// keyed_reverse_500: full order flip of a keyed list — mark-sweep + reorder enforce.
	{
		RuitkBench::Reset();
		FRuitkTestHarness H;
		H.Mount(Ruitk::FC(&BenchKeyedList));
		TArray<double> Us;
		for (int32 r = 0; r < RuitkBench::Reps; ++r)
		{
			const uint64 T0 = FPlatformTime::Cycles64();
			RuitkBench::SetReversed((r % 2) == 0);
			H.Host.PumpFrame();
			Us.Add(RuitkBench::ToUs(FPlatformTime::Cycles64() - T0));
		}
	...
	// mount_unmount_churn_200: cold mount + full teardown of a nested 200-leaf grid.
	{
		...
		for (int32 r = 0; r < RuitkBench::Reps; ++r)
		{
			...
			{
				FRuitkTestHarness H;
				H.Mount(Ruitk::FC(&BenchChurnTree));
			}
			Us.Add(RuitkBench::ToUs(FPlatformTime::Cycles64() - T0));
		}
