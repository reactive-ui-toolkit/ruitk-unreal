...
//
// Ruitk.Widgets.Canvas — the absolute-placement panel (SCanvas wrapper, the Doom-demo
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FRuitkCanvasPanelTest, "Ruitk.Widgets.Canvas",
								 EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
bool FRuitkCanvasPanelTest::RunTest(const FString&)
{
	IRuitkElementAdapter* Adapter = Ruitk::Slate::FindAdapter(Ruitk::InternElementType(FName(TEXT("Canvas"))));
	if (!TestNotNull(TEXT("Canvas adapter registered"), Adapter))
	{
		return false;
	}

	FRuitkEmptyProps Props;
	...
	FRuitkStyleDict SlotA;
	SlotA.Add(FName(TEXT("slot.position")), FRuitkValue(FVector2D(20.0, 30.0)));
	SlotA.Add(FName(TEXT("slot.size")), FRuitkValue(FVector2D(160.0, 90.0)));
	Adapter->InsertChild(Canvas, A, -1, &SlotA);

	TSharedRef<SWidget> B = SNew(SSpacer);
	FRuitkStyleDict SlotB;
	SlotB.Add(FName(TEXT("slot.position")), FRuitkValue(FString(TEXT("5,7"))));
	SlotB.Add(FName(TEXT("slot.size")), FRuitkValue(FString(TEXT("64,64"))));
	...
	FRuitkStyleDict Moved;
	Moved.Add(FName(TEXT("slot.position")), FRuitkValue(FVector2D(21.0, 30.0)));
	Moved.Add(FName(TEXT("slot.size")), FRuitkValue(FVector2D(160.0, 92.0)));
