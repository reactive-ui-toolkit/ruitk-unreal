...
//
// Ruitk.Widgets.NumericEntryBox — TD-012 tail: the controlled numeric field. Uses the headless
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FRuitkNumericEntryBoxTest, "Ruitk.Widgets.NumericEntryBox",
								 EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
bool FRuitkNumericEntryBoxTest::RunTest(const FString&)
{
	IRuitkElementAdapter* Adapter = Ruitk::Slate::FindAdapter(Ruitk::Slate::NumericEntryBoxType());
	...
	SRuitkNumericEntryBox& Entry = static_cast<SRuitkNumericEntryBox&>(Widget.Get());
	...
	if (FSlateApplication::IsInitialized())
	{
		...
		{
			Win.PumpGeometry();
			SWidget* Inner = RuitkTest::FindDescendantByType(Widget.Get(), FName(TEXT("SEditableText")));
			if (TestNotNull(TEXT("inner editable text found"), Inner))
			{
				...
				FRuitkNumericEntryBoxProps Next = Props;
				Next.SetValue(7.0f);
