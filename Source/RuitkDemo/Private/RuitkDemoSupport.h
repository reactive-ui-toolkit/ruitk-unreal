...
namespace RuitkDemo
{
	...
	inline TRuitkContext<FLinearColor> GDemoThemeCtx(FLinearColor(0.23f, 0.65f, 0.95f), FName(TEXT("DemoTheme")));
	...
	// One process-wide URuitkSignalViewModel, GC-rooted by the strong ptr: the UmgHostDemo button
	...
	inline URuitkSignalViewModel* GetSharedVm()
	{
		static TStrongObjectPtr<URuitkSignalViewModel> Vm;
		if (!Vm.IsValid())
		{
			Vm.Reset(NewObject<URuitkSignalViewModel>());
		}
		return Vm.Get();
	}
