...
namespace RUI
{
	...
	RUITKCORE_API FRuiElementTypeId InternElementType(FName Tag);

	/** Lookup without interning (invalid id if unknown). */
	RUITKCORE_API FRuiElementTypeId FindElementType(FName Tag);

	/** Reverse lookup for diagnostics ("Button", "Text", ...). NAME_None for invalid. */
	RUITKCORE_API FName GetElementTypeName(FRuiElementTypeId Id);
