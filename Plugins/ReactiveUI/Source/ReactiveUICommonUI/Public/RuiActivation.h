...
struct RUITKCOMMONUI_API FRuiActivationState
{
...
struct RUITKCOMMONUI_API FRuiFocusTargetRegistry
{
...
namespace RUI::CommonUI
{
	...
	RUITKCOMMONUI_API TRuiContext<FRuiActivationState>& ActivationContext();

	/** Read the full activation state at this point in the tree. */
	RUITKCOMMONUI_API FRuiActivationState UseActivation(FRuiContext& Ctx);

	/** Sugar: is the hosting screen active? */
	RUITKCOMMONUI_API bool UseIsActive(FRuiContext& Ctx);

	/** Sugar: the current input method. */
	RUITKCOMMONUI_API ERuiInputMethod UseInputMethod(FRuiContext& Ctx);

	/** Provide `State` to `Children` (what the activatable host wraps the component in). */
	RUITKCOMMONUI_API FRuiNode ActivationProvider(FRuiActivationState State,
	...
	RUITKCOMMONUI_API TRuiContext<TSharedPtr<FRuiFocusTargetRegistry>>& FocusTargetContext();
	...
	RUITKCOMMONUI_API void UseDesiredFocus(FRuiContext& Ctx, TFunction<void()> FocusAction);

	/** Provide `Registry` to `Children` (what URuiActivatableScreen wraps the component in). */
	RUITKCOMMONUI_API FRuiNode FocusTargetProvider(TSharedPtr<FRuiFocusTargetRegistry> Registry,
														TArray<FRuiNode> Children = TArray<FRuiNode>(),
														FRuiKey Key = FRuiKey());
} // namespace RUI::CommonUI
