...
TMap<FString, FString> FUetkxDriver::BuildAggregators(const TArray<FString>& UetkxPaths)
{
	...
	for (const TPair<FString, TArray<FString>>& Pair : ByModuleDir)
	{
		...
		FString Contents;
		Contents += TEXT("// AUTO-GENERATED aggregator - DO NOT EDIT. Regenerated by RuitkCompile; the\n");
		...
		Contents +=
			FUetkxCodegen::IsSellerRepo()
				? TEXT("// Copyright (c) 2026 Yaniv Kalfa. All Rights Reserved.\n\n")
				: TEXT("// Generated by Reactive UI Toolkit — this generated aggregator belongs to your project.\n\n");
		Contents += TEXT("#include \"CoreMinimal.h\"\n");
		Contents += TEXT("#include \"RuitkContext.h\"\n");
		Contents += TEXT("#include \"RuitkCoreElements.h\"\n");
		Contents += TEXT("#include \"RuitkSignal.h\"\n");
		Contents += TEXT("#include \"RuitkSlateElements.h\"\n");
		Contents += TEXT("#include \"RuitkStyle.h\"\n");
		Contents += TEXT("#include \"RuitkRouter.h\"\n"); // RuitkCore/Public — hard dep, always available
		// INCLUDE_RETIREMENT_PLAN.md §A: the rest are optional-module or engine headers, guarded
		// so a consuming module that depends only on RuitkCore+RuitkSlate (no UMG/
		...
		Contents +=
			TEXT("#if __has_include(\"RuitkAssetBrush.h\")\n#include \"RuitkAssetBrush.h\"\n#endif\n"); // RuitkUMG
		Contents += TEXT("#if __has_include(\"RuitkFieldHooks.h\")\n#include \"RuitkFieldHooks.h\"\n#endif\n");
		Contents += TEXT("#if __has_include(\"RuitkUmgElement.h\")\n#include \"RuitkUmgElement.h\"\n#endif\n");
		Contents +=
			TEXT("#if __has_include(\"RuitkSignalViewModel.h\")\n#include \"RuitkSignalViewModel.h\"\n#endif\n");
		Contents += TEXT("#if __has_include(\"RuitkHostWidget.h\")\n#include \"RuitkHostWidget.h\"\n#endif\n");
		Contents += TEXT("#if __has_include(\"RuitkWorldSubsystem.h\")\n#include \"RuitkWorldSubsystem.h\"\n#endif\n");
		Contents +=
			TEXT("#if __has_include(\"RuitkActivation.h\")\n#include \"RuitkActivation.h\"\n#endif\n"); // RuitkCommonUI
		Contents +=
			TEXT("#if __has_include(\"RuitkActivatableScreen.h\")\n#include \"RuitkActivatableScreen.h\"\n#endif\n");
		Contents += TEXT(
		...
		Contents += TEXT("#define RUITK_UETKX_DECL_PHASE\n");
		for (const FString& Uetkx : Ordered)
		{
			Contents += FString::Printf(TEXT("#include \"%s\"\n"), *RelOf(Uetkx));
		}
		Contents += TEXT("#undef RUITK_UETKX_DECL_PHASE\n");
		for (const FString& Uetkx : Ordered)
		{
			Contents += FString::Printf(TEXT("#include \"%s\"\n"), *RelOf(Uetkx));
		}
...
FUetkxCheckResult FUetkxDriver::CheckDrift(const TArray<FString>& Roots)
{
	...
	for (const FString& Path : All)
	{
		...
		{
			++Out.Drift;
			Out.Messages.Add(FString::Printf(TEXT("%s: generated output missing — run -run=RuitkCompile"), *Path));
		}
	...
	for (const TPair<FString, FString>& Pair : BuildAggregators(All))
	{
		...
		{
			++Out.Drift;
			Out.Messages.Add(FString::Printf(TEXT("%s: aggregator missing — run -run=RuitkCompile"), *Pair.Key));
		}
	...
	// cycle passes `-check` but fails the real RuitkCompile (bughunt CHECK-1).
