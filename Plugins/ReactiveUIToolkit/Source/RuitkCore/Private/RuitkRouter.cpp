// Copyright (c) 2026 Yaniv Kalfa. All Rights Reserved.

#include "RuitkRouter.h"

#include "RuitkContext.h"
#include "RuitkContextHandle.h"

DEFINE_LOG_CATEGORY_STATIC(LogRuiRouter, Log, All);

// ─────────────────────────────────────────────────────────────────────────────────────────
// Pure helpers — path matching / parsing (the router_match suite hits these directly)
// ─────────────────────────────────────────────────────────────────────────────────────────

namespace
{
	/** Split a pathname into non-empty segments (leading/trailing/duplicate slashes ignored). */
	TArray<FString> Segments(const FString& Path)
	{
		TArray<FString> Out;
		Path.ParseIntoArray(Out, TEXT("/"), /*CullEmpty*/ true);
		return Out;
	}

	FString NormalizePathname(const FString& In)
	{
		const TArray<FString> Segs = Segments(In);
		if (Segs.Num() == 0)
		{
			return TEXT("/");
		}
		return TEXT("/") + FString::Join(Segs, TEXT("/"));
	}

	/** Index of the first `?` or `#` in a to-string (whichever is earlier), or INDEX_NONE. */
	int32 TailStart(const FString& To)
	{
		int32 Q, H;
		const bool bQ = To.FindChar(TEXT('?'), Q);
		const bool bH = To.FindChar(TEXT('#'), H);
		if (bQ && bH)
		{
			return FMath::Min(Q, H);
		}
		return bQ ? Q : (bH ? H : INDEX_NONE);
	}

	/** The ?search#hash tail of a to-string (so it can be re-appended once after path resolution). */
	FString ExtractQueryHash(const FString& To)
	{
		const int32 Cut = TailStart(To);
		return Cut == INDEX_NONE ? FString() : To.RightChop(Cut);
	}

	/** The PATHNAME part of a to-string, with any ?query#hash tail removed — what ResolvePath must see.
	 *  ResolvePath segments only on `/`, so passing the raw `To` embeds the tail into the resolved path
	 *  AND lets the caller append it again -> doubled search/hash (bughunt B2). Strip first. */
	FString StripQueryHash(const FString& To)
	{
		const int32 Cut = TailStart(To);
		return Cut == INDEX_NONE ? To : To.Left(Cut);
	}
} // namespace

FRuitkPathMatch Ruitk::MatchPath(const FString& Pattern, const FString& Pathname, bool bEnd)
{
	FRuitkPathMatch Match;
	const TArray<FString> PatSegs = Segments(Pattern);
	const TArray<FString> PathSegs = Segments(Pathname);

	TArray<FString> Consumed;
	int32 i = 0;
	for (int32 p = 0; p < PatSegs.Num(); ++p)
	{
		const FString& Seg = PatSegs[p];
		if (Seg == TEXT("*"))
		{
			// Splat: capture the rest of the path as the "*" param.
			TArray<FString> Rest;
			for (int32 k = i; k < PathSegs.Num(); ++k)
			{
				Rest.Add(PathSegs[k]);
				Consumed.Add(PathSegs[k]);
			}
			Match.Params.Add(TEXT("*"), FString::Join(Rest, TEXT("/")));
			i = PathSegs.Num();
			Match.bMatched = true;
			Match.Pathname = NormalizePathname(FString::Join(Consumed, TEXT("/")));
			return Match; // a splat is always terminal
		}
		if (i >= PathSegs.Num())
		{
			return Match; // pattern longer than path -> no match
		}
		if (Seg.StartsWith(TEXT(":")))
		{
			Match.Params.Add(Seg.RightChop(1), PathSegs[i]);
			Consumed.Add(PathSegs[i]);
			++i;
		}
		else if (Seg == PathSegs[i])
		{
			Consumed.Add(PathSegs[i]);
			++i;
		}
		else
		{
			return Match; // literal mismatch
		}
	}
	if (bEnd && i != PathSegs.Num())
	{
		return Match; // leftover path but a leaf pattern -> no full match
	}
	Match.bMatched = true;
	Match.Pathname = NormalizePathname(FString::Join(Consumed, TEXT("/")));
	return Match;
}

TMap<FString, FString> Ruitk::ParseSearch(const FString& Search)
{
	TMap<FString, FString> Out;
	FString S = Search;
	if (S.StartsWith(TEXT("?")))
	{
		S = S.RightChop(1);
	}
	TArray<FString> Pairs;
	S.ParseIntoArray(Pairs, TEXT("&"), true);
	for (const FString& Pair : Pairs)
	{
		// First value wins for a repeated key (documented contract, RuitkRouter.h) — insert only when
		// absent (bughunt B3: FindOrAdd + assign was last-value-wins, contradicting the header).
		FString Key, Value;
		if (Pair.Split(TEXT("="), &Key, &Value))
		{
			if (!Out.Contains(Key))
			{
				Out.Add(Key, Value);
			}
		}
		else if (!Pair.IsEmpty() && !Out.Contains(Pair))
		{
			Out.Add(Pair, FString());
		}
	}
	return Out;
}

FString Ruitk::BuildSearch(const TMap<FString, FString>& Params)
{
	if (Params.Num() == 0)
	{
		return FString();
	}
	TArray<FString> Keys;
	Params.GetKeys(Keys);
	Keys.Sort(); // stable order
	TArray<FString> Pairs;
	for (const FString& Key : Keys)
	{
		Pairs.Add(Key + TEXT("=") + Params[Key]);
	}
	return TEXT("?") + FString::Join(Pairs, TEXT("&"));
}

FRuitkLocation Ruitk::ParseLocation(const FString& Href)
{
	FRuitkLocation Loc;
	FString Rest = Href;

	int32 HashIdx;
	if (Rest.FindChar(TEXT('#'), HashIdx))
	{
		Loc.Hash = Rest.RightChop(HashIdx);
		Rest = Rest.Left(HashIdx);
	}
	int32 QIdx;
	if (Rest.FindChar(TEXT('?'), QIdx))
	{
		Loc.Search = Rest.RightChop(QIdx);
		Rest = Rest.Left(QIdx);
	}
	Loc.Pathname = NormalizePathname(Rest);
	return Loc;
}

FString Ruitk::ResolvePath(const FString& To, const FString& From)
{
	if (To.StartsWith(TEXT("/")))
	{
		return NormalizePathname(To); // absolute
	}
	// Relative: `From` is treated as the BASE directory — its segments are kept IN FULL and the
	// relative segments append onto them (`child` from `/a/b` -> `/a/b/child`); `..` pops, `.` skips.
	// (Intentionally does NOT drop From's last segment — resolution anchors on the current location as
	// a directory; locked by the router resolve test. RECON-4: the old comment claimed the opposite.)
	TArray<FString> Base = Segments(From);
	TArray<FString> Rel = Segments(To);
	for (const FString& Seg : Rel)
	{
		if (Seg == TEXT(".."))
		{
			if (Base.Num() > 0)
			{
				Base.Pop();
			}
		}
		else if (Seg != TEXT("."))
		{
			Base.Add(Seg);
		}
	}
	return NormalizePathname(FString::Join(Base, TEXT("/")));
}

// ─────────────────────────────────────────────────────────────────────────────────────────
// Context values + handles
// ─────────────────────────────────────────────────────────────────────────────────────────

namespace
{
	struct FRuitkBlockerEntry
	{
		TSharedRef<bool> bActive;
		TFunction<void(const FString&)> OnBlocked;
		FRuitkBlockerEntry() : bActive(MakeShared<bool>(false)) {}
	};
	using FRuitkBlockerList = TArray<TSharedPtr<FRuitkBlockerEntry>>;

	/** Navigation + location context (provided by <Router>). Functions are EXCLUDED from equality
	 *  so consumers re-render only when the location/nav data changes, not every Router render. */
	struct FRuitkRouterCtxValue
	{
		bool bInRouter = false;
		FRuitkLocation Location;
		ERuitkNavigationType NavType = ERuitkNavigationType::Push;
		bool bCanGoBack = false;
		TFunction<void(const FString&, bool)> Navigate;
		TFunction<void(int32)> Go;
		TSharedPtr<FRuitkBlockerList> Blockers;

		bool operator==(const FRuitkRouterCtxValue& O) const
		{
			return bInRouter == O.bInRouter && Location == O.Location && NavType == O.NavType &&
				   bCanGoBack == O.bCanGoBack;
		}
		bool operator!=(const FRuitkRouterCtxValue& O) const { return !(*this == O); }
	};

	/** Per-matched-route context (params + the nested Outlet element). */
	struct FRuitkRouteCtxValue
	{
		TMap<FString, FString> Params;
		TSharedPtr<FRuitkNode> Outlet; // the child element to render at UseOutlet()

		bool operator==(const FRuitkRouteCtxValue& O) const
		{
			return Params.OrderIndependentCompareEqual(O.Params) && Outlet == O.Outlet;
		}
		bool operator!=(const FRuitkRouteCtxValue& O) const { return !(*this == O); }
	};

	const TRuitkContext<FRuitkRouterCtxValue>& RouterContext()
	{
		static const TRuitkContext<FRuitkRouterCtxValue> Ctx(FRuitkRouterCtxValue{}, FName(TEXT("RuitkRouter")));
		return Ctx;
	}
	const TRuitkContext<FRuitkRouteCtxValue>& RouteContext()
	{
		static const TRuitkContext<FRuitkRouteCtxValue> Ctx(FRuitkRouteCtxValue{}, FName(TEXT("RuitkRoute")));
		return Ctx;
	}

	static const TMap<FString, FString> GEmptyParams;

	/** The in-memory history the Router mutates through a stable ref. */
	struct FRuitkHistory
	{
		TArray<FRuitkLocation> Back;
		FRuitkLocation Current;
		TArray<FRuitkLocation> Forward;
		ERuitkNavigationType NavType = ERuitkNavigationType::Push;
		int32 Serial = 0;
	};
} // namespace

// ─────────────────────────────────────────────────────────────────────────────────────────
// <Router> — the in-memory history provider
// ─────────────────────────────────────────────────────────────────────────────────────────

namespace
{
	struct FRuitkRouterProps final : public FRuitkPropsBase
	{
		FString InitialPath = TEXT("/");
		virtual bool Equals(const FRuitkPropsBase& Other) const override
		{
			return BaseFieldsEqual(Other) && InitialPath == static_cast<const FRuitkRouterProps&>(Other).InitialPath;
		}
	};

	bool AnyBlockerActive(const TSharedPtr<FRuitkBlockerList>& Blockers, const FString& Href)
	{
		if (!Blockers.IsValid())
		{
			return false;
		}
		for (const TSharedPtr<FRuitkBlockerEntry>& Entry : *Blockers)
		{
			if (Entry.IsValid() && *Entry->bActive)
			{
				if (Entry->OnBlocked)
				{
					Entry->OnBlocked(Href);
				}
				return true;
			}
		}
		return false;
	}

	FRuitkNodeArray RouterComp(FRuitkContext& Ctx, const FRuitkRouterProps& Props, const TArray<FRuitkNode>& Children)
	{
		TSharedRef<TRuitkRef<FRuitkHistory>> HistRef = Ctx.UseRef<FRuitkHistory>();
		TSharedRef<TRuitkRef<bool>> Primed = Ctx.UseRef<bool>(false);
		if (!Primed->Current)
		{
			Primed->Current = true;
			HistRef->Current.Current = Ruitk::ParseLocation(Props.InitialPath);
		}
		TSharedRef<TRuitkRef<FRuitkBlockerList>> Blockers = Ctx.UseRef<FRuitkBlockerList>();

		TTuple<int32, TRuitkSetter<int32>> Version = Ctx.UseState<int32>(0);
		const TRuitkSetter<int32>& Bump = Version.Value;

		TSharedPtr<FRuitkBlockerList> BlockerList(Blockers, &Blockers->Current); // alias ptr sharing HistRef lifetime

		auto Navigate = [HistRef, BlockerList, Bump](const FString& To, bool bReplace)
		{
			FRuitkHistory& H = HistRef->Current;
			// Resolve only the pathname part, then re-append the ?query#hash tail ONCE (bughunt B2).
			const FString Resolved = Ruitk::ResolvePath(StripQueryHash(To), H.Current.Pathname);
			const FRuitkLocation Next =
				Ruitk::ParseLocation(To.StartsWith(TEXT("/")) ? To : Resolved + ExtractQueryHash(To));
			if (AnyBlockerActive(BlockerList, Next.ToHref()))
			{
				return;
			}
			if (bReplace)
			{
				H.Current = Next;
				H.NavType = ERuitkNavigationType::Replace;
			}
			else
			{
				H.Back.Add(H.Current);
				H.Current = Next;
				H.NavType = ERuitkNavigationType::Push;
			}
			H.Forward.Reset();
			Bump([](const int32& V) { return V + 1; });
		};

		auto Go = [HistRef, BlockerList, Bump](int32 Delta)
		{
			FRuitkHistory& H = HistRef->Current;
			// A back/forward (POP) navigation is subject to blockers too — the back button must not
			// bypass a UseBlocker guard that Navigate honors (bughunt RECON-2). Veto on the immediate
			// target before touching the stacks.
			FString TargetHref;
			if (Delta < 0 && H.Back.Num() > 0)
			{
				TargetHref = H.Back.Last().ToHref();
			}
			else if (Delta > 0 && H.Forward.Num() > 0)
			{
				TargetHref = H.Forward[0].ToHref();
			}
			if (TargetHref.IsEmpty() || AnyBlockerActive(BlockerList, TargetHref))
			{
				return;
			}
			if (Delta < 0)
			{
				for (int32 n = 0; n < -Delta && H.Back.Num() > 0; ++n)
				{
					H.Forward.Insert(H.Current, 0);
					H.Current = H.Back.Pop();
				}
			}
			else if (Delta > 0)
			{
				for (int32 n = 0; n < Delta && H.Forward.Num() > 0; ++n)
				{
					H.Back.Add(H.Current);
					H.Current = H.Forward[0];
					H.Forward.RemoveAt(0);
				}
			}
			H.NavType = ERuitkNavigationType::Pop;
			Bump([](const int32& V) { return V + 1; });
		};

		FRuitkRouterCtxValue Value;
		Value.bInRouter = true;
		Value.Location = HistRef->Current.Current;
		Value.NavType = HistRef->Current.NavType;
		Value.bCanGoBack = HistRef->Current.Back.Num() > 0;
		Value.Navigate = MoveTemp(Navigate);
		Value.Go = MoveTemp(Go);
		Value.Blockers = BlockerList;
		Ctx.ProvideContext(RouterContext(), MoveTemp(Value));

		return Children;
	}
	RUITK_COMPONENT(RouterComp)
} // namespace

FRuitkNode Ruitk::Router(TArray<FRuitkNode> Children, FString InitialPath, FRuitkKey Key)
{
	FRuitkRouterProps Props;
	Props.InitialPath = MoveTemp(InitialPath);
	return Ruitk::FC<FRuitkRouterProps>(&RouterComp, MoveTemp(Props), MoveTemp(Children), Key);
}

// ─────────────────────────────────────────────────────────────────────────────────────────
// Route matching tree + <Routes> / UseRoutes / UseOutlet
// ─────────────────────────────────────────────────────────────────────────────────────────

namespace
{
	using Ruitk::FRuitkRoute;

	int32 SpecificityScore(const FRuitkRoute& Route)
	{
		// static segments rank highest, params next, splats last, longer patterns first.
		int32 Score = 0;
		for (const FString& Seg : Segments(Route.Path))
		{
			if (Seg == TEXT("*"))
			{
				Score += 1;
			}
			else if (Seg.StartsWith(TEXT(":")))
			{
				Score += 3;
			}
			else
			{
				Score += 5;
			}
		}
		if (Route.bIndex)
		{
			Score += 2;
		}
		return Score;
	}

	/** Remove the matched prefix from RemainingPath, returning the leftover (relative) pathname. */
	FString StripPrefix(const FString& RemainingPath, const FString& MatchedPathname)
	{
		const TArray<FString> Rem = Segments(RemainingPath);
		const int32 Consumed = Segments(MatchedPathname).Num();
		TArray<FString> Left;
		for (int32 i = Consumed; i < Rem.Num(); ++i)
		{
			Left.Add(Rem[i]);
		}
		return NormalizePathname(FString::Join(Left, TEXT("/")));
	}

	/** Depth-first best match. Fills Chain (parent..leaf) + merged Params. */
	bool MatchInto(const TArray<FRuitkRoute>& Routes, const FString& RemainingPath, TMap<FString, FString>& Params,
				   TArray<const FRuitkRoute*>& Chain)
	{
		TArray<const FRuitkRoute*> Ranked;
		for (const FRuitkRoute& R : Routes)
		{
			Ranked.Add(&R);
		}
		Ranked.Sort([](const FRuitkRoute& A, const FRuitkRoute& B) { return SpecificityScore(A) > SpecificityScore(B); });

		const bool bAtEnd = Segments(RemainingPath).Num() == 0;
		for (const FRuitkRoute* Route : Ranked)
		{
			if (Route->bIndex)
			{
				if (bAtEnd)
				{
					Chain.Add(Route);
					return true;
				}
				continue;
			}
			const bool bLayout = Route->Children.Num() > 0;
			const FRuitkPathMatch M = Ruitk::MatchPath(Route->Path, RemainingPath, /*bEnd*/ !bLayout);
			if (!M.bMatched)
			{
				continue;
			}
			if (bLayout)
			{
				// Stage this branch's params/children so a non-matching branch rolls back cleanly.
				const FString Rest = StripPrefix(RemainingPath, M.Pathname);
				TArray<const FRuitkRoute*> ChildChain;
				TMap<FString, FString> ChildParams = M.Params;
				const bool bChildMatched = MatchInto(Route->Children, Rest, ChildParams, ChildChain);
				// A layout with LEFTOVER path that no child consumed does NOT match here — let a sibling
				// (e.g. a catch-all `*`) try, instead of silently swallowing the trailing segments and
				// shadowing that sibling (bughunt RECON-1). Rest empty = the layout's own index/outlet.
				if (!Rest.IsEmpty() && !bChildMatched)
				{
					continue;
				}
				Params.Append(ChildParams);
				Chain.Add(Route);
				Chain.Append(ChildChain);
				return true;
			}
			Params.Append(M.Params);
			Chain.Add(Route);
			return true;
		}
		return false;
	}

	/** Wraps a matched route's Element: provides {params, outlet} then renders the element. */
	struct FRuitkRouteHostProps final : public FRuitkPropsBase
	{
		TSharedPtr<FRuitkRouteCtxValue> Ctx;
		TSharedPtr<FRuitkNode> Element;
		virtual bool Equals(const FRuitkPropsBase& Other) const override
		{
			const FRuitkRouteHostProps& O = static_cast<const FRuitkRouteHostProps&>(Other);
			return BaseFieldsEqual(Other) && Element == O.Element &&
				   ((Ctx == O.Ctx) || (Ctx.IsValid() && O.Ctx.IsValid() && *Ctx == *O.Ctx));
		}
	};

	FRuitkNodeArray RouteHostComp(FRuitkContext& Ctx, const FRuitkRouteHostProps& Props, const TArray<FRuitkNode>&)
	{
		if (Props.Ctx.IsValid())
		{
			Ctx.ProvideContext(RouteContext(), *Props.Ctx);
		}
		return {Props.Element.IsValid() ? *Props.Element : Ruitk::Fragment({})};
	}
	RUITK_COMPONENT(RouteHostComp)

	FRuitkNode WrapRoute(const TSharedPtr<FRuitkRouteCtxValue>& RouteCtx, const FRuitkNode& Element)
	{
		FRuitkRouteHostProps Props;
		Props.Ctx = RouteCtx;
		Props.Element = MakeShared<FRuitkNode>(Element);
		return Ruitk::FC<FRuitkRouteHostProps>(&RouteHostComp, MoveTemp(Props), {}, FRuitkKey());
	}
} // namespace

FRuitkNode UseRoutes(FRuitkContext& Ctx, const TArray<Ruitk::FRuitkRoute>& RouteList)
{
	const FRuitkLocation& Loc = UseLocation(Ctx);

	TMap<FString, FString> Params;
	TArray<const Ruitk::FRuitkRoute*> Chain;
	if (!MatchInto(RouteList, Loc.Pathname, Params, Chain) || Chain.Num() == 0)
	{
		return Ruitk::Fragment({});
	}

	// Build leaf -> root: each level provides the SAME merged params + the child as its Outlet.
	FRuitkNode Element = Ruitk::Fragment({}); // the leaf's outlet is empty
	for (int32 i = Chain.Num() - 1; i >= 0; --i)
	{
		TSharedPtr<FRuitkRouteCtxValue> RouteCtx = MakeShared<FRuitkRouteCtxValue>();
		RouteCtx->Params = Params;
		RouteCtx->Outlet = MakeShared<FRuitkNode>(Element);
		Element = WrapRoute(RouteCtx, Chain[i]->Element);
	}
	return Element;
}

FRuitkNode Ruitk::Routes(TArray<FRuitkRoute> RouteList, FRuitkKey Key)
{
	// A tiny component so UseRoutes runs inside the render tree (hooks-legal) under the Router.
	struct FRoutesProps final : public FRuitkPropsBase
	{
		TSharedPtr<TArray<FRuitkRoute>> List;
		virtual bool Equals(const FRuitkPropsBase& Other) const override
		{
			return BaseFieldsEqual(Other) && List == static_cast<const FRoutesProps&>(Other).List;
		}
	};
	struct FLocal
	{
		static FRuitkNodeArray Comp(FRuitkContext& Ctx, const FRoutesProps& P, const TArray<FRuitkNode>&)
		{
			return {P.List.IsValid() ? UseRoutes(Ctx, *P.List) : Ruitk::Fragment({})};
		}
	};
	static const FName Id = Ruitk::RegisterComponentId((void*)&FLocal::Comp, FName(TEXT("RuitkRoutesComp")));
	(void)Id;
	FRoutesProps Props;
	Props.List = MakeShared<TArray<FRuitkRoute>>(MoveTemp(RouteList));
	return Ruitk::FC<FRoutesProps>(&FLocal::Comp, MoveTemp(Props), {}, Key);
}

FRuitkNode UseOutlet(FRuitkContext& Ctx)
{
	const FRuitkRouteCtxValue& Route = Ctx.UseContext(RouteContext());
	return Route.Outlet.IsValid() ? *Route.Outlet : Ruitk::Fragment({});
}

// ─────────────────────────────────────────────────────────────────────────────────────────
// <Link>
// ─────────────────────────────────────────────────────────────────────────────────────────

namespace
{
	struct FRuitkLinkProps final : public FRuitkPropsBase
	{
		FString To;
		bool bReplace = false;
		virtual bool Equals(const FRuitkPropsBase& Other) const override
		{
			const FRuitkLinkProps& O = static_cast<const FRuitkLinkProps&>(Other);
			return BaseFieldsEqual(Other) && To == O.To && bReplace == O.bReplace;
		}
	};

	FRuitkNodeArray LinkComp(FRuitkContext& Ctx, const FRuitkLinkProps& Props, const TArray<FRuitkNode>& Children)
	{
		// The core Link just forwards its children; a host wrapper (Slate Button/Text) binds the
		// click to the navigate captured here. We expose navigate through context so the wrapper
		// can call it — but for the pure-core node we simply pass children through (the Slate
		// layer's Ruitk::Slate::Link binds OnClicked). Kept minimal + host-agnostic.
		(void)Ctx;
		(void)Props;
		return Children;
	}
	RUITK_COMPONENT(LinkComp)
} // namespace

FRuitkNode Ruitk::Link(FString To, TArray<FRuitkNode> Children, bool bReplace, FRuitkKey Key)
{
	FRuitkLinkProps Props;
	Props.To = MoveTemp(To);
	Props.bReplace = bReplace;
	return Ruitk::FC<FRuitkLinkProps>(&LinkComp, MoveTemp(Props), MoveTemp(Children), Key);
}

// ─────────────────────────────────────────────────────────────────────────────────────────
// The 17 hooks
// ─────────────────────────────────────────────────────────────────────────────────────────

bool UseInRouterContext(FRuitkContext& Ctx)
{
	return Ctx.UseContext(RouterContext()).bInRouter;
}

const FRuitkLocation& UseLocation(FRuitkContext& Ctx)
{
	return Ctx.UseContext(RouterContext()).Location;
}

FString UsePathname(FRuitkContext& Ctx)
{
	return UseLocation(Ctx).Pathname;
}

FString UseSearch(FRuitkContext& Ctx)
{
	return UseLocation(Ctx).Search;
}

ERuitkNavigationType UseNavigationType(FRuitkContext& Ctx)
{
	return Ctx.UseContext(RouterContext()).NavType;
}

TFunction<void(const FString&, bool)> UseNavigate(FRuitkContext& Ctx)
{
	const FRuitkRouterCtxValue& R = Ctx.UseContext(RouterContext());
	if (R.Navigate)
	{
		return R.Navigate;
	}
	return [](const FString&, bool) {};
}

TFunction<void(int32)> UseGo(FRuitkContext& Ctx)
{
	const FRuitkRouterCtxValue& R = Ctx.UseContext(RouterContext());
	if (R.Go)
	{
		return R.Go;
	}
	return [](int32) {};
}

TTuple<bool, TFunction<void()>> UseBackStack(FRuitkContext& Ctx)
{
	const FRuitkRouterCtxValue& R = Ctx.UseContext(RouterContext());
	TFunction<void(int32)> Go = R.Go;
	return MakeTuple(R.bCanGoBack, TFunction<void()>(
									   [Go]()
									   {
										   if (Go)
										   {
											   Go(-1);
										   }
									   }));
}

const TMap<FString, FString>& UseParams(FRuitkContext& Ctx)
{
	return Ctx.UseContext(RouteContext()).Params;
}

TTuple<TMap<FString, FString>, TFunction<void(TMap<FString, FString>)>> UseSearchParams(FRuitkContext& Ctx)
{
	const FRuitkRouterCtxValue& R = Ctx.UseContext(RouterContext());
	TMap<FString, FString> Current = Ruitk::ParseSearch(R.Location.Search);
	const FString Pathname = R.Location.Pathname;
	TFunction<void(const FString&, bool)> Navigate = R.Navigate;
	TFunction<void(TMap<FString, FString>)> Set = [Navigate, Pathname](TMap<FString, FString> Next)
	{
		if (Navigate)
		{
			// PUSH a new history entry (React-Router's setSearchParams default), so back restores
			// the prior search state instead of it being lost to an in-place replace (bughunt RECON-3).
			Navigate(Pathname + Ruitk::BuildSearch(Next), /*bReplace*/ false);
		}
	};
	return MakeTuple(MoveTemp(Current), MoveTemp(Set));
}

FRuitkPathMatch UseMatch(FRuitkContext& Ctx, const FString& Pattern)
{
	return Ruitk::MatchPath(Pattern, UseLocation(Ctx).Pathname, /*bEnd*/ true);
}

bool UseIsActive(FRuitkContext& Ctx, const FString& Pattern, bool bEnd)
{
	return Ruitk::MatchPath(Pattern, UseLocation(Ctx).Pathname, bEnd).bMatched;
}

FString UseResolvedPath(FRuitkContext& Ctx, const FString& To)
{
	return Ruitk::ResolvePath(To, UseLocation(Ctx).Pathname);
}

FString UseHref(FRuitkContext& Ctx, const FString& To)
{
	if (To.StartsWith(TEXT("/")))
	{
		return Ruitk::ParseLocation(To).ToHref();
	}
	// Resolve only the pathname, then append the tail once (bughunt B2 — was doubling via
	// UseResolvedPath(To) embedding the tail AND `+ Tail` re-appending it).
	return Ruitk::ResolvePath(StripQueryHash(To), UseLocation(Ctx).Pathname) + ExtractQueryHash(To);
}

void UseBlocker(FRuitkContext& Ctx, bool bBlock, TFunction<void(const FString&)> OnBlocked)
{
	const FRuitkRouterCtxValue& R = Ctx.UseContext(RouterContext());
	TSharedPtr<FRuitkBlockerList> List = R.Blockers;

	// A stable entry, registered on mount / unregistered on unmount; bActive refreshed each render.
	TSharedRef<TRuitkRef<TSharedPtr<FRuitkBlockerEntry>>> EntryRef = Ctx.UseRef<TSharedPtr<FRuitkBlockerEntry>>();
	if (!EntryRef->Current.IsValid())
	{
		EntryRef->Current = MakeShared<FRuitkBlockerEntry>();
	}
	EntryRef->Current->OnBlocked = OnBlocked;
	*EntryRef->Current->bActive = bBlock;

	TSharedPtr<FRuitkBlockerEntry> Entry = EntryRef->Current;
	Ctx.UseEffect(
		[List, Entry]() -> FRuitkEffectCleanup
		{
			if (List.IsValid())
			{
				List->Add(Entry);
			}
			return [List, Entry]()
			{
				if (List.IsValid())
				{
					List->Remove(Entry);
				}
			};
		},
		Ruitk::Deps()); // mount-only registration; bActive/OnBlocked mutate in place each render
}
