...
//
// `-run=RuitkMigrateImports` — the one-shot, IDEMPOTENT, re-runnable codemod (A8d) that migrates a
...
/** `-run=RuitkMigrateEsModules` — the ES-modules codemod (ES_MODULES_EXECUTION_PLAN.md M7, G-10):
 ...
 *  3. wrapper rewrite: `component Name(P: T = D)` → `FRuitkNode Name(T P = D)` (param colon-flip
