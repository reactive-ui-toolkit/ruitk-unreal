...
//
// HMR v2 Phase 3 (D-HMR-6) — the two RuitkUetkx editor commands, the C++ sibling of the Unity
...
// FRuitkUetkxInputProcessor makes them fire globally (any focused editor window), the way Unity's
...
class FRuitkUetkxCommands : public TCommands<FRuitkUetkxCommands>
{
public:
	FRuitkUetkxCommands();
...
class FRuitkUetkxInputProcessor : public IInputProcessor
{
public:
	virtual ~FRuitkUetkxInputProcessor() override = default;

	virtual void Tick(const float, FSlateApplication&, TSharedRef<ICursor>) override {}
	virtual bool HandleKeyDownEvent(FSlateApplication& SlateApp, const FKeyEvent& KeyEvent) override;
	virtual const TCHAR* GetDebugName() const override { return TEXT("RuitkUetkxInput"); }
};
