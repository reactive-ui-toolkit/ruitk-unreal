...
Editor support for **`.uetkx`**, the JSX-like markup of [ReactiveUI for Unreal](https://github.com/reactive-ui-toolkit/ruitk-unreal) — a React-style reactive UI library for Unreal Engine 5.6+, in pure C++.
...
- Malformed style/slot value strings, flag-form misuse, string/flag props on typed component params, and Ref without {expr} all diagnose live (2311/0105); the schema now carries attrKinds.
- Live 0109/0110/0111 mirrors (duplicate attributes, duplicate sibling keys, parent-ignored slot keys — parent-tracked incl. directive bodies), UETKX2312 event-payload misuse (void/mismatched/silently-empty reads), and UETKX2313 sinceUE-vs-EngineAssociation gating.
- Brush-name validation live (2311 + did-you-mean against the engine's resolvable style set) and value-completion parity: FCoreStyle brush names inside BorderImage="", true/false for bool-kind values.
- Instant 2310 for butchered ctor-style declarations (the DoomFace blind spot) with a looser did-you-mean threshold; UETKX0112 canonical-casing mirror; attr-name completion fixed to whole-token replacement (no more slot.Clipping hybrids; the Slot. prefix narrows to slot keys).
- Completion/diagnostic parity: slot-key suggestions filter by the parent's consumption map (labeled "read by <parent>"), and already-present attrs or sinceUE-gated tags are never offered — accepting a suggestion can no longer produce an immediate error.
- Live UETKX2305 strict-usage mirror: a usage whose name a workspace file exports but this file never imports errors as-you-type with the add-import quick-fix (kind-matched, sidecar-deduped); 2310 near-miss candidates extend to import names and same-file declarations.
- FILE_SCOPED_EXPORTS mirrors: same-name export pairs across files publish zero diagnostics (the live 2106 mirror is retired); 2305 add-import suggestions and go-to-exporter pick the importer-NEAREST exporter exactly like the compiler; the rename refusal for names exported elsewhere is retired.
