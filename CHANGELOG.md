...
- **BREAKING — full identifier rebrand.** Plugin folder + descriptor:
  ...
  zips: `ReactiveUIToolkit-<ver>[.zip|-UE<eng>.zip]`. Console variables: `rui.*` →
  `ruitk.*` (update your ini/console lines). Editor identity: `stat Ruitk`, Message Log
  page id `"Ruitk"` (label "Reactive UI Toolkit"), preview tab id `RuitkPreview`, loc
  namespace `"Ruitk"`. Baked codegen constants: `*_RUI_HOOK_SIG` → `*_RUITK_HOOK_SIG`
  (regenerated by `RuitkCompile`).
